...
def update_display(subject_name, dice, jaccard): 
...
def display_3d_mask_plot():
    ...
    if binary_mask_1 is not None and binary_mask_2 is not None:
        ...
        ax1.scatter(x1, y1, z1, marker='.', color='red')

        x2, y2, z2 = binary_mask_2.nonzero()
        ax2.scatter(x2, y2, z2, marker='.', color='blue')
...
def batch_processing(directory_path_1, directory_path_2, seg_label_1, seg_label_2):
    ...
    for i in range(num_files): 
        ...
        if os.path.isfile(file_path_1) and os.path.isfile(file_path_2): 
            ...
            update_display(subject_name, dice, jaccard)
...
frame_left.place(relx=0.5, rely=0.4, anchor=tk.CENTER)


frame_middle = tk.Frame(root)
frame_middle.grid(row=0, column=1, padx=10, pady=10)

frame_right = tk.Frame(root)
frame_right.grid(row=0, column=2, padx=10, pady=10)

frame_bottom = tk.Frame(root)
frame_bottom.grid(row=1, column=1, columnspan=3, padx=10, pady=10)
...
frame_bottom.rowconfigure(1, minsize=1)
...
display_panel = tk.Text(frame_middle, height=15, width=80)
display_panel.grid(row=1, column=0, padx=5, pady=5)  # Update the row number to position it below the canvas
set_style(display_panel)
